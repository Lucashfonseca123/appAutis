...
const PlayerScreen = () => {
  ...
  const [visibleModalUpgrade, setVisibleModalUpgrade] = useState<boolean>(
    false
  );
  ...
  const currentProgress = useSelector((appState: AppState) =>
    appState.PlayerFeature.menuReducer.currentStage[idMenu]
      ? appState.PlayerFeature.menuReducer.currentStage[idMenu].progress
      : 0
  );

  const currentStageDone = useSelector((appState: AppState) =>
    appState.PlayerFeature.menuReducer.currentStage[idMenu]
      ? appState.PlayerFeature.menuReducer.currentStage[idMenu].done
      : false
  );
  ...
  const missingStage = useSelector(
    (appState: AppState) =>
      appState.AccreditFeature.accreditReducer.missingStage
  );
  ...
  return (
    <Container>
      ...
      <DivButton>
        {alternativesAnswer.map((res) => (
          <Button
            text={res}
            onPress={() => sendAnswer(res)}
            fontSize={14}
            heightSize={10}
            widthSize={95}
          />
      ...
      <Modal
        ...
        typeMax={
          messageButton === 'Próximo' ? 'FelizOrelhaBaixoDente' : 'Feliz'
        }>
